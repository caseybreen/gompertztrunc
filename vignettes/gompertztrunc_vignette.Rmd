...
What's the effect of owning a home adjusting for education level and urban/rural? 

```{r}
## Example with only one cohort
home_ownership <- gompertz_mle(formula = death_age ~ ownhome + urban + educ_yrs, 
                                   data = dmf_demo,
                                   left_trunc = 1975 , 
                                   right_trunc = 2005)
...
## education years factor 
dmf_demo <- dmf_demo %>% 
  mutate(educ_yrs_factor = as.factor(educ_yrs))

home_ownership <- gompertz_mle(formula = death_age ~ educ_yrs_factor, 
                                   data = dmf_demo,
                                   left_trunc = 1975 , 
                                   right_trunc = 2005)
...
## look at effect on e65
convert_hazards_to_ex(home_ownership$results, use_model_estimates = T) %>% 
  mutate(term = socviz::prefix_strip(parameter, "as.factor")) %>% 
  ggplot(aes(x = term, y = e65)) + 
  geom_point() + 
  theme_cowplot()
```
## Person-weights 

The Gompertztrunc package can incorporate person-level weights. Here, we will simulate person weights. 

For example, let's assume that our data represents people who owned homes over the age of 90. To account for this, we assign person weights. 
...
## Association between home ownership and death age 
education_gradient <- gompertz_mle(formula = death_age ~ ownhome, 
                                   data = dmf_demo,
                                   left_trunc = 1975 , 
                                   right_trunc = 2005)

## Rerun with weights 
education_gradient_weighted <- gompertz_mle(formula = death_age ~ ownhome, 
                                   data = dmf_demo,
                                   left_trunc = 1975, 
                                   right_trunc = 2005,
                                   weight = pweight)
...
education_gradient_weighted$results 
```
Did the weighting change our substantive conclusions? 
