...
## education gradient 
tic()
results_optim <- gompertz_mle(formula = death_age ~ educ_yrs, left_trunc = 1975, right_trunc = 2005, data = dmf_staircase)
toc()


## Run-Time: 5 hours. 18398.488 sec elapsed
tic()
results_staircase <- gompertz_mle(formula = death_age ~ educ_yrs_character, left_trunc = 1975, right_trunc = 2005, data = dmf_staircase)
toc()
...
gompertz_mle_results <- convert_hazards_to_e65(df = results_optim) %>% 
  mutate(parameter = prefix_strip(parameter, "educ_yrs")) %>% 
=======
...
>>>>>>> 72a2a4cb0e6f6e72b7d6ca8318660629949959fa
...
tic()
bni_baseline_model <- gompertz_mle(formula = death_age ~ bni_std + birth_order, data = bni_analysis_file, lower = 1988, upper = 2005)
toc()
...
tic()
bni_sibfe_model <- gompertz_mle(formula = death_age_demean ~ bni_std_demean + birth_order, data = bni_analysis_file, lower = 1988, upper = 2005)
toc()
...
## Run-Time: 5 hours. 18398.488 sec elapsed
tic()
results_optim <- gompertz_mle(formula = death_age ~ educ_yrs_character, lower = 1988, upper = 2005, data = numident_staircase)
toc()
