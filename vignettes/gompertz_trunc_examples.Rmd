...
tic()
tgm(aod ~ temp + sex + isSouth + sex:isSouth, data = data.trunc)
toc()
...
education_staircase <- gompertz_mle(fml = death_age ~ as.factor(educ_yrs), data = censoc_example, left_trunc = 1975, right_trunc = 2005)
...
1: apply to real data (e.g., BUNMD) or one of Censoc datasets

- Seems to work okay on very simple DMF dataset. I'm curious how it will work on BUNMD or Numident. 

2:  Get function to work with weights 

3:  check if CI is correct 
...
4:  include case where we see death_age as whole numbers, not continuous.

5:  include way to specify structure of "beta", e.g. different for men and women, or smooth function of cohort or ... 
