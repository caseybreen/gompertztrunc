...
mle_results <- convert_hazards_to_e65(gompert_mle_results$results, use_model_estimates = T)
...
```{r, eval = F}
## now add a covariate  
gompert_mle_results_cov <- gompertz_mle(fml = death_age ~ bpl_string + age_first_application, 
                                    left_trunc = 1988,
                                    right_trunc = 2005,
                                    data = bunmd_demo)
...
mle_results <- convert_hazards_to_e65(gompert_mle_results_cov$results, use_model_estimates = T)
```
...
education_gradient <- gompertz_mle(fml = death_age ~ _____, 
                                   data = dmf_demo,
                                   left_trunc = _____ , 
                                   right_trunc = _____ )
...
convert_hazards_to_e65(education_gradient$results, use_model_estimates = T)
```
