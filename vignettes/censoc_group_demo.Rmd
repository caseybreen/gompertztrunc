...
It assumes that the underlying mortality distribution follows a Gompertz distribution: 
...
- $h(x)$ hazard at age x 
- $\alpha$ is some baseline level of mortality 
- $\beta$ gives rate of increase of mortality 

The main function in the package is the `gompertztrunc::gompertz_mle`. The `gompertztrunc::gompertz_mle` function takes the following arguments: 

- `fml`: model formula (for example, death_age ~ educ_yrs + homeownership)
...
## return a list 
simulated_example <- gompertztrunc::gompertz_mle(fml = aod ~ temp + sex + isSouth,
                                                 left_trunc = 1888,
                                                 right_trunc = 1905,
                                                 data = sim_data)
...
## convert to e65
convert_hazards_to_e65(df = simulated_example$results, use_model_estimates = T)
```
...
A 95% confidence interval should in the long run have a coverage ratio of 95%: approximately 95% of the confidence intervals should contain the true value. 

We can assess the coverage ratio of our confidence intervals by simulating new data and seeing how often our confidence intervals contain the true value. 
...
confidence_interval_coverage <- bind_rows(results_interaction)
...
```{r}
model <- lm(death_age ~ bpl_string + as.factor(byear), data = bunmd_demo)
...
```{r}
## Gompertz Trunc 
## Use 1988-2005 because we are 
gompert_mle_results <- gompertz_mle(fml = death_age ~ bpl_string, 
                                    left_trunc = 1988,
                                    right_trunc = 2005,
                                    data = bunmd_demo)

mle_results <- convert_hazards_to_e65(gompert_mle_results$results)
...
education_gradient <- gompertz_mle(fml = death_age ~ educ_yrs + urban + ownhome, data = censoc_example, left_trunc = 1975, right_trunc = 2005)
