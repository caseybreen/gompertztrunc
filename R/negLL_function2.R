...
negLL_function2 <- function(par, y, X, y.left, y.right, wt, log.b.start, b0.start) {

  ## note exp(par) just gets them back to original scale
  b <- exp(log.b.start)
  names(b) <- "b"
  # b0.start <- par[2]
  b0.start <- c(b0.start = b0.start)

  beta_coef <- c(b0.start, par)
  ## in terms of log effect, so no
  ## transformation needed.
  log.A <- X %*% cbind(beta_coef) ## add up log effects
